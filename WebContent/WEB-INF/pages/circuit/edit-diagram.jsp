...
<body>
	<div id="toobar"
		style="overflow: hidden; position: relative; top: 0px; left: 0px;">
		<input type="text" value="开关名/开关编号" /> <input type="button"
			value="查找开关" onclick="" /> <input type="radio" name="zoom" value="1"
			checked="checked" />放大 <input type="radio" name="zoom" value="0" />缩小
		(<span style="color: gray">选中 放大或缩小单选，然后双击电路图</span>) 编辑:<input
			type="checkbox" value="0" id="editable"
			onchange="esm.circuit.draw.edit(this)" />
	</div>
  <svg id="cableDiagram" viewBox="0 0 1599 899" width="1599" height="899" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="none" style="background-color:black"> 
   ...
   <image x="40.5" y="93.5" width="20" height="15" xlink:href="/images/yj_close.gif" transform="matrix(3.344,0,0,2.005,788.949,37.196)" id="YJBDZ" title="YJBDZ" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" />
	...
	<div id="addLineDialog" class="easyui-dialog" style="width:600px;height:300px"
data-options="title:'线路基本信息',closed:true,buttons:[{
			text:'保存',
			handler:function(){
			
			  //发送ajax请求，把用户填写的数据提交到服务器端
			   var postData = new Object();
			   postData.lineId = jQuery('#lineId').val();
			   if(postData.lineId.trim().length<1){
			   		jQuery.messager.alert('请填写线路编号.');
			   		return ;
			   }
			  	jQuery('#temp').attr('id','line-'+postData.lineId);
			  	alert(jQuery('#temp'));
			   postData.lineName = jQuery('#lineName').val();
			   postData.parentLineId =jQuery('#parentLineId').val();
			   // alert(postData.parentLineId);
			   postData.upstreamSwitchIds = jQuery('#upstreamSwitchIds').val();
			   postData.svg = jQuery('#cableDiagram').html();
			   jQuery.ajax({
			...
			text:'关闭',
			handler:function(){jQuery('#addLineDialog').dialog('close');}
			}]">
		<table>
			<tr>
				<td>线路名:</td>
				<td><input type="text" id="lineName" value=""/></td>
			</tr>
			<tr>
				<td>线路编号:</td>
				<td><input type="text" id="lineId" value=""/></td>
			</tr>
			<tr>
				<td>父线路编号:</td>
				<td><input type="text" id="parentLineId" value=""/></td>
			</tr>
			<tr>
				<td>线路上游开关id:</td>
				<td><input type="text" id="upstreamSwitchIds" value=""/><span style="text-color:gray">您画的这条线的上游第一个开关，如果有多个，则用逗号隔开</span></td>
			</tr>
		</table>
	</div>
</body>
